...
from core.api_config import OPENAI_API_KEY, MODEL_NAME
...
def init_log_path(my_log_path):
    ...
    if log_path:
        ...
        # 另外一个记录api调用的文件
        api_trace_json_path = os.path.join(dir_name if dir_name else '.', 'api_trace.json')
        # Clear log files at start of each run
        open(log_path, 'w', encoding='utf8').close()
        open(api_trace_json_path, 'w', encoding='utf8').close()
        ...
        try:
            if log_path is None:
                ...
                with open(log_path, 'a+', encoding='utf8') as log_fp, open(api_trace_json_path, 'a+', encoding='utf8') as trace_json_fp:
                    print('\n' + '='*60, file=log_fp)
                    print('>>> PROMPT >>>', file=log_fp)
                    print('='*60 + '\n', file=log_fp)
                    print(input_prompt, file=log_fp)
                    print('\n' + '='*60, file=log_fp)
                    print('<<< RESPONSE <<<', file=log_fp)
                    print('='*60 + '\n', file=log_fp)
                    sys_response, prompt_token, response_token = api_func(input_prompt)
                    print(sys_response, file=log_fp)
                    print('\n' + '-'*60, file=log_fp)
                    print(f'Tokens: prompt={prompt_token}, response={response_token}', file=log_fp)
                    print('-'*60 + '\n', file=log_fp)
                    print(f'\n prompt_token,response_token: {prompt_token} {response_token}\n')
...
if __name__ == "__main__":
    res = safe_call_llm('Hello, how are you?')
    print(res)
